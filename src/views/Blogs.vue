    ...
    <div
      ...
      <div v-for="(blog, index) in blogs" :key="index">
        <router-link :to="{name: 'BlogId', params: {id: index}}">
          <img class="mb-5 lg:mb-6 w-full transition-transform duration-300 transform-gpu hover:scale-105" :src="blog.blogImage" :alt="blog.meta" loading="lazy">
        </router-link>
        <router-link :to="{name: 'BlogId', params: {id: index}}">
        ...
        <router-link :to="{name: 'BlogId', params: {id: index}}"
                     class="block w-fit bg-button px-8 py-4 rounded-3xl hover:brightness-105" v-text="'Read Post'"/>
      </div>
    </div>
  </section>
  <section
...
<script setup>
import useBlogs from "../composables/views/useBlogs.js";
